                ...
                while True:  # 只有当前一页 或有下一页 或是最后一页  获取元素源码
                    ...
                    for div in div_list:
                        ...
                        if name == search_name:
                            ...
                            div.find_element_by_xpath('./div[1]/input').send_keys(Keys.SPACE)   # 选中
                            time.sleep(0.3)
                            selected_num += 1
                            exist = True
                    time.sleep(2)
                    if selected_num > 0:  # 选中数量大于0
                        ...
                        driver.find_element_by_xpath('//input[@value="new_price"]').send_keys(Keys.SPACE)   # 选中
                        time.sleep(0.5)
                        if sign == '-':
                            top_price = price
                            low_price = price2
                            ...
                            top_price = price1
                            low_price = price2
                        new_price = format(round(random.uniform(low_price, top_price), 2), '0.2f')
...
def main():
    ...
    # df['added'] = 0  # 已复制个数
    df['price1'] = df.apply(lambda x: round(x['price'] * (1 + x['floating']), 2), axis=1)
    ...
    a_list_num = 0
    start_time = datetime.datetime.now()
    start(pic_wait, s_date, e_date, df[:5].copy(), key_words, file_path[:-5] + start_time.strftime('%Y%m%d %H-%M-%S') + 'res.xlsx', a_list_num, domain[i], chromedriver_path, chrome_port)
