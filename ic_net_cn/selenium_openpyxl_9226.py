...
def get_res(start_url, s_date, e_date, df, key_words, res_path, res_wk, res_sheet, amount, domain, driver, handles):
    ...
    for key_word in key_words[:amount]:
        ...
        li_list = driver.find_elements_by_xpath('//ul[@id="resultList"]/li[@class="stair_tr"]')[1:]
        ...
        if search_num > 49: # 一页最多49
            search_num = 49
        search_title = li_list[0].find_element_by_xpath('./div[@class="result_id"]').get_attribute('title')
        if (num != search_num) or (key_word != search_title):
...
if __name__ == '__main__':
    ...
    df = pd.read_excel(file_path).iloc[4000:5000]
    df = df[['品牌', '型号']]
    ...
    i = 0
    start(s_date, e_date, df, key_words, file_path[:-5] + 'res' + str(5 * amount) + '.xlsx', amount, domain[i], chromedriver_path, chrome_port)
