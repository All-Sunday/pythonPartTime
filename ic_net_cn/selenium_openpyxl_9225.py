...
if __name__ == '__main__':
    ...
    df = pd.read_excel(file_path).iloc[3000:4000]
    df = df[['品牌', '型号']]
    ...
    i = 0
    start(s_date, e_date, df, key_words, file_path[:-5] + 'res' + str(4 * amount) + '.xlsx', amount, domain[i], chromedriver_path, chrome_port)
