...
def init(start_url, domain, res_path, chromedriver_path, chrome_port):
    ...
    if not os.path.exists(res_path):
        ...
        res_sheet.append(['土地名称', '区域', '板块', '开发商', '受让方', '土地用途', '成交时间', '成交情况', '成交总价(万元)', '出让底价(万元)', '用地面积(㎡)', '容积率', '对应项目名称', '开盘时间', '每月成交面积', '每月成交价格'])
...
def get_res(start_url, s_date, e_date, res_path, res_wk, res_sheet, domain, driver, handles):
    ...
    for i in range(100):
        time.sleep(0.5)
        land_trs_list = driver.find_elements_by_xpath('//div[@class="el-dialog__body"]//table[@class="el-table__body"][1]/tbody/tr')
...
if __name__ == '__main__':
    file_path = r'E:\code_workplace\python\2022\january\data\vip_cric_com\res.xlsx'
    ...
    domain = ['https://vip.cric.com/project/landMarket?cityName=%E5%B9%BF%E5%B7%9E']
