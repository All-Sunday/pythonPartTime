# 根据商品名直接爬取id id爬price buff需登录cookie
import sys
import datetime
import json
import time
...
def query_buff_detail(id, name, header):
    ...
    try:
        # print(requests.get(api_url, headers=header))
...
def main():
    ...
    while True:
        ...
        df = pd.read_excel(source_path, header=None)[190:]
...
def get_res2(name, headers_youpin, headers_buff):
    sign, commodity_id = query_youyou2(name, headers_youpin)

    if sign:
        sign, buff_id = query_buff2(name, headers_buff)
        if sign:
            return 1, commodity_id, buff_id
        elif 'Login Required' in buff_id:
            print('***********************************')
            print('***********************************')
            return 2, commodity_id, buff_id
        ...
        print('***********************************')
        print('***********************************')
        return -1, commodity_id, ''


def main2():
    source_path = r'商品列表_id.xlsx'
    f = open("cookie.txt", "r")
    cookie = f.read().strip()
    f.close()
    ua = UserAgent(path=r"fake_useragent.json")
    ...
    # headers_buff.pop('Host')
    # headers_buff.pop('Referer')
    # # print(requests.get('https://www.whatismybrowser.com/', headers=headers_buff).text)
    # print(requests.get('https://www.whatismybrowser.com/detect/what-http-headers-is-my-browser-sending', headers=headers_buff).text)
    # time.sleep(100)
    ...
    # for row in df.itertuples():
    for i in df.itertuples():  # df 遍历 df 修改数据
        # print(i)
        index, name = getattr(i, 'Index'), getattr(i, '_1')  # df 遍历 df.itertuples() 取值

        headers_youpin['User-Agent'] = ua.random
        headers_buff['User-Agent'] = ua.random
        # headers_buff['Cookie'] = cookie
        sign, commodity_id, buff_id = get_res2(name, headers_youpin, headers_buff)
        if sign == 1:
            print(name, commodity_id, buff_id)
            df.loc[index,'_1'] = str(commodity_id)
            df.loc[index,'_2'] = str(buff_id)
        elif sign == 2:
            print(name, commodity_id, buff_id)
            df.loc[index,'_1'] = str(commodity_id)
            if '未搜索到' not in buff_id:
                msg = '程序error'
                print(msg)
                # sign, eamil_res = send_mail(msg, res)
                # print(sign, eamil_res)
                if 'Login Required' in commodity_id:
                    sys.exit()
        else:
            print(name, commodity_id, buff_id)
            if '未搜索到' not in commodity_id:
                msg = '程序error'
                print(msg)
                # sign, eamil_res = send_mail(msg, res)
                # print(sign, eamil_res)
        # print(df)
        # time.sleep(100)
    df.to_excel(source_path.split('.')[0] + 'res.xlsx', index=None)
    end_time = datetime.datetime.now()
    total_time = str(end_time - start_time)
    print(end_time, 'Finish-----------------------共耗时', total_time)
