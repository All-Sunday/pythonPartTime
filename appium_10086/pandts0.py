# @Description: 限制主机 不同主机也只允许一个运行 多线程版
...
# def test(index, phones, cookies, keyword, activity, means, wk, ws, res_path):
def test(index, phones, cookies, keyword, activity, means, wk, res_path):
    ...
    while t_num < phones_num:
        ...
        phone = phones[t_num]
        t_num += 1
        print(name + ', t_num = %d--' % t_num, phone)
        mutex.release()
        # get_res(phone, cookies, keyword, activity, means, wk, ws, res_path)
        # get_res(phone, cookies, keyword, activity, means, wk, res_path)  # ws全局共享变量
