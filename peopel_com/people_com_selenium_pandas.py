...
def init(start_url, domain, chromedriver_path):
    # 启动谷歌浏览器
    options = Options()
...
def get_res1(start_url, s_date, e_date, key_words, a_list_num, domain, driver, handles):
    res = []
    all_a_list = []
    ...
    for key_word in key_words:      # 循环搜索关键词
        ...
        get_data(a_list[a_list_num:], key_word, domain, res, driver, handles)  # 通过 a 链接获取详细信息
    print(res)
...
def get_data(site, a_list, key_word, domain, res, driver, handles):
...
def get_res(start_url, s_date, e_date, key_words, a_list_num, domain, driver, handles):
    # time.sleep(100)
    i=0
    while r'show_more' in driver.page_source:
        print(++i)
        driver.find_element_by_xpath('//div[@id="show_more"]').click()
        # driver.execute_script("arguments[0].click();", driver.find_element_by_xpath('div[@id="show_more"]'))
        time.sleep(2)
    return
def start(s_date, e_date, key_words, a_list_num, domain, chromedriver_path):
    start_time = datetime.datetime.now()
    print(start_time, 'Begin-----------------------')

    start_url = domain
    driver = init(start_url, domain, chromedriver_path)
    driver.implicitly_wait(30)
    handles = driver.window_handles

    get_res(start_url, s_date, e_date, key_words, a_list_num, domain, driver, handles)
...
if __name__ == '__main__':
    ...
    domain = ['http://liuyan.people.com.cn/threads/list?fid=569#state=4']
    ...
    i = 0
    a_list_num = 0
    start(s_date, e_date, key_words[0], a_list_num, domain[i], chromedriver_path)
